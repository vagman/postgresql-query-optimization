...
each type.
*/
SELECT V.id, V.type, count(V.type)
FROM vessels AS V
JOIN positions AS P ON V.id = P.vessel_id
WHERE P.speed > 30
...
FROM positions AS P
JOIN vessels AS V on P.vessel_id = V.id
WHERE V.type BETWEEN '70' and '79'
	AND P.speed = 0.0
	AND P.t >= '2019-08-15 00:00:00' AND P.t <= '2019-08-18 00:00:00'
...
WHERE V.type::integer BETWEEN 70 and 79
	AND P.t >= '2019-08-12 00:00:00' AND P.t <= '2019-08-19 00:00:00'
GROUP BY P.vessel_id
HAVING MAX(P.speed) = 0.0;
